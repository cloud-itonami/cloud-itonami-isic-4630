(ns provisiontrade.operation
  "OperationActor -- one food/beverage/tobacco wholesale operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (ProvisionTradeAdvisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Provision Trading
  ...
  One graph run = one food/beverage/tobacco wholesale operation (intake
  -> advise -> govern -> decide -> commit | hold | approval). No
  unbounded inner loop -- each operation is auditable and checkpointed.
  A provision-order's life is advanced by MANY ops (intake / verify /
  ...
  this node when the governor is clean -- see `provisiontrade.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [provisiontrade.provisiontradeadvisor :as provisiontradeadvisor]
            [provisiontrade.governor :as governor]
            [provisiontrade.phase :as phase]
...
(defn build
  ...
  `provisiontrade.store/Store`).
  opts:
    :advisor      -- a `provisiontrade.provisiontradeadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (provisiontradeadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; ProvisionTradeAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (provisiontradeadvisor/-advise advisor store request)]
            {:proposal p :audit [(provisiontradeadvisor/trace request p)]})))

      ;; Provision Trading Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
