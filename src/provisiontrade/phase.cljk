(ns provisiontrade.phase
  "Phase 0->3 staged rollout for the food/beverage/tobacco wholesale
  actor.

    Phase 0  read-only        -- no writes, still governor-gated.
    Phase 1  assisted-intake  -- provision-order intake allowed, every
                                 write needs human approval.
    Phase 2  assisted-verify  -- adds regulatory-verification writes,
                                 still approval.
  ...
  real food, beverage or tobacco product to a counterparty (an
  autonomous warehouse pallet-picking/staging robot performs the
  physical warehouse handling, or an operator does) and settling a real
  ...
  are always a human trading supervisor's call. `provisiontrade.
  ...
  no-capital-risk lifecycle distinct from the provision-order itself.")

(def read-ops  #{})
(def write-ops #{:order/intake :regulatory/verify :delivery/dispatch :invoice/settle})
...
(def phases
  ...
  {0 {:label "read-only"        :writes #{}                                                :auto #{}}
   ...
   2 {:label "assisted-verify"  :writes #{:order/intake :regulatory/verify}                 :auto #{}}
...
(defn verdict->disposition
  "Map a Provision Trading Governor verdict to a base disposition before
  the phase gate."
  [verdict]
